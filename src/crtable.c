...
void createCrTableMenu(HWND hwnd, HWND *crTableMenuControls, SqlRules *rules)
{
    ...
    crTableMenuControls[5] = CreateWindow("BUTTON", ADDCOLUMN_MSG, STL_BUTTON, 1220, 130, 150, 24, hwnd, (HMENU)ADDCOLUMN_ID, (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
    crTableMenuControls[6] = CreateWindow("BUTTON", ADDTABLE_MSG, STL_BUTTON, 830, 700, 150, 24, hwnd, (HMENU)ADDTABLE_ID, (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
    crTableMenuControls[7] = CreateWindow("BUTTON", ADDFK_MSG, STL_BUTTON, 1220, 160, 150, 24, hwnd, (HMENU)ADDFK_ID, (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
    crTableMenuControls[8] = CreateWindow("BUTTON", DELTABLE_MSG, STL_BUTTON, 670, 700, 150, 24, hwnd, (HMENU)DELTABLE_ID, (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
    crTableMenuControls[9] = CreateWindow("BUTTON", EXPORTMODEL_MSG, STL_BUTTON, 155, 700, 150, 24, hwnd, (HMENU)EXPORTMODEL_ID, (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
    addColumn(hwnd, crTableMenuControls, SET_MENU, rules);
    getTableName(hwnd, buffer);
    addTableToList(hwnd, buffer);
}

void addColumn(HWND hwnd, HWND *crTableMenuControls, int mode, SqlRules *rules)
{
    ...
    if ((mode == COL_ADD || SET_MENU) && columnNumber < rules->maxCol)
    {
